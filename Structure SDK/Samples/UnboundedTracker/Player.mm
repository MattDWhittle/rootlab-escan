...
const float WARP_MOVEMENT_TIME = 0.98;
...
-(id) initWithGameData:(GameData*)gameData
{
    ...
    if (self)
    {
        ...
        self.playerNode = [SCNNode nodeWithGeometry:capsuleGeo];
        [self.playerNode setName:@"playerNode"];
        [self.playerNode setGeometry:capsuleGeo];
        ...
        [self.playerNode setPhysicsBodyAndContactTestBitMaskCollisionCategoryAll:[SCNPhysicsBody dynamicBody]];
        [self.playerNode.physicsBody setPhysicsShape:capPhysShape];
        [self.playerNode.physicsBody setMass: 70];
        [self.playerNode.physicsBody setDamping: 0];
        [self.playerNode.physicsBody setFriction: 0];
        [self.playerNode.physicsBody setRestitution: 0];
        [self.playerNode.physicsBody setVelocityFactor:SCNVector3Make(1.0, 0.0, 1.0)];
        [self.playerNode.physicsBody setAngularVelocityFactor:SCNVector3Make(0, 0, 0)];
        ...
        [self.playerNode addChildNode:self.heightNode];
        ...
        [self.pov.camera setFieldOfView:80.0];
...
-(void) reset
{
    ...
    [self.playerNode.physicsBody setType:SCNPhysicsBodyTypeKinematic];
    
    float desiredYaw = M_PI/2.0;
    if (self.startRoom == 2)
    {
        [self.playerNode setPosition:SCNVector3Make(-30.0, 0.0, 0.0)];
    }
    else // Default game start
    {
        [self.playerNode setPosition:SCNVector3Make(5.0, 0.0, 1.5)];
    }
    ...
    [self.playerNode.physicsBody setType:SCNPhysicsBodyTypeDynamic];
    [self.playerNode.physicsBody setVelocity:SCNVector3Zero];
...
-(void) updateWithTrackerPose:(const TrackerUpdate&) trackerUpdate locked:(BOOL)isLocked deltaTime:(float)time;
{
    ...
    if (_isWarping)
    {
        if (_warpFinishTime > CACurrentMediaTime())
        {
            if(_shouldStartWarp)
            {
                SCNVector3 _warpStartPosition = [SCNTools getWorldPos:self.playerNode];
            ...
            float easeMagnitude = 8*(0.5 - fabs(0.5 - (_warpFinishTime - CACurrentMediaTime())/WARP_MOVEMENT_TIME));
    ...
    // Physics object (playerNode) does not move in y, the heightNode does
    instantVelocity.y = 0;
    ...
    [self.playerNode.physicsBody setVelocity:instantVelocity];
    ...
    if(_releaseGrabbedObject)
    {
        ...
        [SCNTransaction setCompletionBlock:^{
            [self->_gameData.view.scene.rootNode addChildNode:self->_lastGrabbedObject];
            [self->_lastGrabbedObject.physicsBody setType:SCNPhysicsBodyTypeDynamic];
            [self->_lastGrabbedObject.physicsBody setVelocity:self->_attachedObjectVelocity];
        }];
...
-(void) updateSceneKit
{
    ...
    //    CGPoint center;
    if (isnan(_gameData.viewCenter.x))
    {
        CGPoint pt;
        pt.x = _gameData.view.window.bounds.size.width*0.5;
        pt.y = _gameData.view.window.bounds.size.height*0.5;
        
        _gameData.viewCenter = pt;
    }
    
    NSDictionary* opt = nil;
    if (@available(iOS 11.0, *))
         opt = @{SCNHitTestOptionSearchMode : @1};
    NSArray *hitResults  = [_gameData.view hitTest:_gameData.viewCenter options:opt] ;
    ...
    for (SCNHitTestResult *result in hitResults)
    {
        ...
        if (castRayLength < closestDistance)
        {
            ...
            if (_isWarpEnabled && _isWarpPossible && !_isGrabPossible && !_isHoldingObject)
            {
                ...
                SCNVector3 nodePosition =[[self.playerNode presentationNode] position];
