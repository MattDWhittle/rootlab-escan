            ...
            switch (currentCommand.action)
            {
                ...
                case TrackerCommand::ActionSetInitialPose:
                {
                    _tracker.initialCameraPose = currentCommand.cameraPose;
                    {
                        ...
                        _lastUpdate.trackerHints = { .trackerIsLost = YES, .sceneIsTooClose = NO, .modelOutOfView = NO };
                        _lastUpdate.poseAccuracy = STTrackerPoseAccuracyNotAvailable;
...
//for debugging tracker performance
-(void)debugTrackerComputeInterval:(float)lastTrackerComputeInterval
{
    const int INTERVAL_COUNT = 100;
    static double trackerComputeCumulative = 0;
    static int trackerComputeCount = 0;

    trackerComputeCumulative += lastTrackerComputeInterval/INTERVAL_COUNT;
    trackerComputeCount ++;
    if (trackerComputeCount > INTERVAL_COUNT)
    {
//        NSLog(@"TrackerThread: tracker Compute Avg Duration %lf s", trackerComputeCumulative);
        
        trackerComputeCumulative = 0;
        trackerComputeCount = 0;
    }
}
...
-(void) estimateNewPose:(const TrackerCommand&)command
{
    ...
    double preTrackerCompute = nowInSeconds();
    ...
    double postTrackerCompute = nowInSeconds();
    ...
    newTrackerUpdate.trackerHints = _tracker.trackerHints;
    newTrackerUpdate.poseAccuracy = _tracker.poseAccuracy;
    newTrackerUpdate.trackingError = trackerError;
    
    // When the quality was poor, we could still get a pose.
    newTrackerUpdate.couldEstimatePose = trackingOk;
