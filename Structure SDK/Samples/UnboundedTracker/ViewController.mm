...
- (void)renderer:(id <SCNSceneRenderer>)aRenderer didSimulatePhysicsAtTime:(NSTimeInterval)time
{
    ...
    if (_slamState.lastSceneKitTrackerUpdateProcessed.trackerHints.sceneIsTooClose)
    {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self->_captureSession.sensorMode == STCaptureSessionSensorModeReady &&
            self->_slamState.lastSceneKitTrackerUpdateProcessed.poseAccuracy != STTrackerPoseAccuracyNotAvailable &&
            [self trackingConsideredBad])
        {
            NSMutableAttributedString *attributedMessageText;
            
            if (self->_slamState.lastSceneKitTrackerUpdateProcessed.trackerHints.sceneIsTooClose)
            {
                SCNVector3 lookVector = [SCNTools getLookAtVectorOfNode:self->_gameData.player.pov];
...
{
    if (!latencies)
        latencies = [[NSMutableArray alloc] init];
    
    double didLatency = nowInSeconds() - _slamState.trackerThread.lastUpdate.timestamp;
    
    [latencies addObject:[NSNumber numberWithDouble:didLatency]];
    ...
    if (latencies.count > 100)
    {
        ...
        {
            latencyAvg += ((NSNumber*)latency).doubleValue;
        }
        latencyAvg /= latencies.count;
//        NSLog(@"did latencyAvg %lf", latencyAvg);
        
        [latencies removeAllObjects];
    }
}
...
{
    // avoid setting if it is already in expected state
    if (!self.trackingLostLabel.hidden)
