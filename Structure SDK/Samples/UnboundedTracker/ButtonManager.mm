...
-(id) initWithGameData:(GameData*)gameData
{
    ...
    if(self)
    {
        ...
        [self.redButton setPhysicsBodyAndContactTestBitMaskCollisionCategoryAll:redBoxBody];
        [self.blueButton setPhysicsBodyAndContactTestBitMaskCollisionCategoryAll:blueBoxBody];
...
-(void) physicsWorld:(SCNPhysicsWorld *)world didBeginContact:(SCNPhysicsContact *)contact isGrabbingObject:(BOOL)isGrabbingObject
{
    ...
    if(nodeA == _gameData.player.playerNode && [nodeB.name containsString:@"LoadingBay_A_Button"])
    {
...
-(void) pressRedButton
{
    ...
    [SCNTransaction setCompletionBlock :^{
        [self->_gameData.cubeManager dropFirstRoomCube];
        [self->_redFloorMaterial.diffuse setContents:[UIColor redColor]];
        [self openHatch];
    }];
    {
        float deltaY = -0.2;
...
-(void) pressBlueButton
{
    ...
    [SCNTransaction setAnimationTimingFunction : [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
    [SCNTransaction setCompletionBlock :^{
        [self->_gameData.doorManager openLoadingBayDoors];
        [self->_redFloorMaterial.diffuse setContents:[UIColor whiteColor]];
        self->_redFloorGlow = NO;
        [self->_redFloorMaterial.emission setContents:[UIColor blackColor]];
    }];
    {
