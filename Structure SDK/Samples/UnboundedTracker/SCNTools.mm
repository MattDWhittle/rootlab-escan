...
#pragma mark - SCNNode extensions

@implementation SCNNode(StarOps)

-(void)setPhysicsBodyAndContactTestBitMaskCollisionCategoryAll:(SCNPhysicsBody*)physicsBody
{
    //starting in iOS 9, we must set contactTestBitMask manually to get any collisions
    // this property does not exist in iOS 8.
    if (([[[UIDevice currentDevice] systemVersion] compare:@"9.0" options:NSNumericSearch] != NSOrderedAscending))
    {
        physicsBody.contactTestBitMask = SCNPhysicsCollisionCategoryAll;
    }
...
+ (SCNScene*)loadSceneFromPlist:(NSString *)list
{
    ...
    for (SCNNode *nextRoom in roomArray)
    {
        ...
        if (collisionNodes.count > 0)
        {
            ...
            [roomNode setPhysicsBodyAndContactTestBitMaskCollisionCategoryAll:physicsBody];
        }
