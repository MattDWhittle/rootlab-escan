...
- (void)setupGL
{
    ...
    {
        // We used VGA depth when we setup capture session
        const size_t w = 640;
        const size_t h = 480;
...
- (void)renderColorImage
{
    ...
    glDisable(GL_BLEND);
    [_display.yCbCrTextureShader useShaderProgramWithUndistortion:&_display.intrinsics];
...
{
    if (![depthFrame hasValidExtrinsics]) { return; }
